...
function point2circle(data,r){
    let result = [];
    var temp = [];
    for (i=0;i<data.length;i++){
        temp = [];
        if (i ==9){
            temp.push({x:(data[i].x - r),y:(data[i].y -r),z:(data[i].z)});
            temp.push({x:(data[i].x + r),y:(data[i].y -r),z:(data[i].z)});
            temp.push({x:(data[i].x + r),y:(data[i].y +r),z:(data[i].z)});
            temp.push({x:(data[i].x - r),y:(data[i].y +r),z:(data[i].z)});
...
var waternodescircle = point2circle(waternodesdata,0.05);
var pump=point2circle(pumpdata,0.15);
var testtextdata = data_point1;
var DSRcircle = point2circle(DSRposition,0.15);
...
function processData_text(data,cur_id,textdata){
    ...
    recs
        ...
        .attr('y', function(d){ return origin[1] + 100 * d.centroid.y })
        .classed('_3d',true)
